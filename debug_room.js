...
async function debugRoom(roomCode) {
  ...
  try {
    ...
    const gameState = await getGameById(
      (await db('games').where({ room_code: roomCode }).select('id').first())
        ?.id
    );
    ...
    console.log(
      `  Current Player Position: ${gameState.currentPlayerPosition}`
    );
    ...
    if (currentPlayerPos !== null && currentPlayerPos !== undefined) {
      ...
      if (currentPlayer) {
        ...
        console.log(
          `   Can Act: ${currentPlayer.status === 'active' && currentPlayer.chips > 0 ? 'Yes' : 'No'}`
        );
      }
    } else {
      console.log(
    ...
    if (allInPlayers.length > 0) {
      ...
      allInPlayers.forEach((p) => {
        console.log(
          `   ${p.name} (P${p.position}) - $${p.chips} chips, bet=$${p.currentBet}`
        );
      });
    }
    ...
    if (gameState.status === 'completed') {
      ...
      console.log(
        `   This game should allow starting the next hand or ending the session`
      );
      if (
        ...
        console.log(
          `   ⚠️  WARNING: Current player position should be null when game is completed`
        );
      } else {
      ...
      if (currentPlayerPos !== null && currentPlayerPos !== undefined) {
        ...
        if (currentPlayer) {
          if (currentPlayer.status === 'all_in') {
            console.log(
              `⚠️  PROBLEM: Current player is ALL_IN but game says it's their turn!`
            );
            console.log(`   This should have been normalized.`);
          } else if (currentPlayer.status === 'folded') {
            console.log(
              `⚠️  PROBLEM: Current player has FOLDED but game says it's their turn!`
            );
            console.log(`   This should have been normalized.`);
          } else if (currentPlayer.chips === 0) {
            console.log(
              `⚠️  WARNING: Current player has $0 chips but status is ${currentPlayer.status}`
            );
          } else {
            console.log(
              `✓ Current player can act (${currentPlayer.name} is ${currentPlayer.status} with $${currentPlayer.chips})`
            );
          }
        }
      }
