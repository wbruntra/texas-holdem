  ...
  try {
    ...
    for (const index of indexes) {
      if (index.unique && !index.name.startsWith('sqlite_autoindex')) {
        const indexInfoResult = await knex.raw(
    ...
    throw new Error(
      `Failed to generate DDL for table ${tableName}: ${error.message}`
    );
  }
};
...
const generateModelFile = async (tableName) => {
  ...
  try {
    ...
    if (fs.existsSync(filePath)) {
      ...
      if (endIndex === -1) {
        console.error(
          `DDL end marker not found in ${filePath}. Cannot update.`
        );
        return;
      }
      ...
      if (endIndex <= startIndex) {
        console.error(
          `Invalid marker positions in ${filePath}. Cannot update.`
        );
        return;
      }
      ...
      const newLines = lines
        ...
        .concat(newDdlLines, lines.slice(endIndex));
