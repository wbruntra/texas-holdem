  ...
  try {
    ...
    for (const pid of playerIds) {
      ...
      if (joinEvent && joinEvent.payload) {
        const payload =
          typeof joinEvent.payload === 'string' ? JSON.parse(joinEvent.payload) : joinEvent.payload
      ...
      // In this system, event playerId IS the room_player_id
      const roomPlayerId = pid
      ...
      await db('game_players').insert({
        // id: pid, // REMOVED - column doesn't exist
        game_id: gameId,
    ...
    gameState.players.forEach((p: any) => {
      console.log(
        ` - ${p.name} (ID: ${p.id}): ${p.chips} chips, ${p.status}, Bet: ${p.currentBet}`,
      )
    })

    // Simple assertions based on typical game flow logic validity
    if (gameState.pot < 0) throw new Error('Pot is negative')
    if (gameState.players.some((p: any) => p.chips < 0))
      throw new Error('Player has negative chips')
