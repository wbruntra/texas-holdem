...
describe('Game Service', () => {
  ...
  test('throws error when starting game with fewer than 2 players', async () => {
    ...
    await expect(gameService.startGame(game.id)).rejects.toThrow(
...
describe('Player Service', () => {
  ...
  test('prevents duplicate player names', async () => {
    ...
    await expect(
      playerService.joinGame(game.id, 'Alice', 'pass1')
  ...
  test('prevents joining game that has started', async () => {
    ...
    await expect(
      playerService.joinGame(game.id, 'Charlie', 'pass3')
  ...
  test('enforces 10 player limit', async () => {
    ...
    await expect(
      playerService.joinGame(game.id, 'Player11', 'password')
  ...
  test('authenticates player with correct password', async () => {
    ...
    const authenticated = await playerService.authenticatePlayer(
      game.id,
      'Alice',
      'mypassword'
    );
  ...
  test('completes full hand to showdown', async () => {
    ...
    async function completeBettingRound() {
      ...
      while (
        ...
        iterationCount++;
        const currentPlayer =
          gameState.players[gameState.currentPlayerPosition];
