...
describe('Player Service', () => {
  ...
  test('prevents duplicate player names', async () => {
    ...
    await expect(playerService.joinGame(game.id, 'Alice', 'pass1')).rejects.toThrow('already taken');
  });
  ...
  test('prevents joining game that has started', async () => {
    ...
    await expect(playerService.joinGame(game.id, 'Charlie', 'pass3')).rejects.toThrow('already started');
  });
  ...
  test('requires password with minimum length', async () => {
    ...
    await expect(
      playerService.joinGame(game.id, 'Alice', 'abc')
...
describe('Action Service', () => {
  ...
  test('rejects action when not player\'s turn', async () => {
    ...
    await expect(
      actionService.submitAction(playerObj.id, 'fold')
  ...
  test('gets valid actions for current player', async () => {
    ...
    const actions = await actionService.getPlayerValidActions(
      playerObj.id
    );
  ...
  test('advances to flop after preflop betting complete', async () => {
    ...
    gameState = await actionService.submitAction(
      playerObj1.id,
      'call'
    );
    ...
    async function completeBettingRound() {
      ...
      while (
        ...
        if (actions.canCheck) {
          ...
          throw new Error(`No valid check or call action for player at position ${gameState.currentPlayerPosition}`);
        }
      }
