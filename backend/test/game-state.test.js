...
describe('Betting Logic - Action Validation', () => {
  test('validates fold action', () => {
    ...
    const result = validateAction(state, state.currentPlayerPosition, ACTION_TYPE.FOLD)
...
describe('Betting Logic - Action Processing', () => {
  ...
  test('processes call action', () => {
    ...
    expect(state.players[currentPos].currentBet).toBeGreaterThanOrEqual(callAmount)
  ...
  test('processes raise action', () => {
    ...
    expect(state.players[currentPos].chips).toBe(initialChips - callAmount - raiseAmount)
...
describe('Game State Machine - Showdown', () => {
  test('determines winner and distributes pot', () => {
    ...
    state.players[0].holeCards = [createCard('A', 'hearts'), createCard('A', 'diamonds')]
    state.players[1].holeCards = [createCard('K', 'hearts'), createCard('K', 'diamonds')]
  ...
  test('splits pot on tie', () => {
    ...
    // Both have same hand
    state.players[0].holeCards = [createCard('A', 'hearts'), createCard('K', 'hearts')]
    state.players[1].holeCards = [createCard('A', 'diamonds'), createCard('K', 'diamonds')]
