...
const { shuffleDeck, createDeck, dealHoleCards, determineWinners, evaluateHand } = require('./poker-engine');
...
function startNewHand(state) {
  ...
  const smallBlindPosition = isHeadsUp ? dealerPosition : getNextActivePosition(players, dealerPosition);
  ...
  const activePlayerIndices = players
    ...
    .filter(({ player }) => player.status === PLAYER_STATUS.ACTIVE || player.status === PLAYER_STATUS.ALL_IN)
    .map(({ index }) => index);
...
function shouldAutoAdvance(state) {
  ...
  const canBet = activePlayers.filter(p => 
    p.status === PLAYER_STATUS.ACTIVE && p.chips > 0
  );
