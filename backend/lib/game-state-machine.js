...
function startNewHand(state) {
  const activePlayers = state.players.filter(
    (p) => p.chips > 0 && p.status !== PLAYER_STATUS.OUT
  );
  ...
  const smallBlindAmount = Math.min(
    players[smallBlindPosition].chips,
    state.smallBlind
  );
  const bigBlindAmount = Math.min(
    players[bigBlindPosition].chips,
    state.bigBlind
  );
...
function advanceRound(state) {
  ...
  const allInPlayers = state.players.filter(
    (p) => p.status === PLAYER_STATUS.ALL_IN
  );
...
function processShowdown(state) {
  ...
  let pots =
    state.pots && state.pots.length > 0
      ? state.pots
      : calculatePots(state.players);
  ...
  const gameStatus =
    playersWithChips.length <= 1 ? GAME_STATUS.COMPLETED : state.status;
