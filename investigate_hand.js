...
async function investigateHand(roomCode) {
  ...
  try {
    ...
    if (gameState.communityCards && gameState.communityCards.length > 0) {
      ...
      console.log(
        `  No community cards (hand hasn't reached flop or cards were cleared)`
      );
    }
    ...
    if (gameState.status === 'completed') {
      ...
      if (gameState.currentRound === 'flop') {
        console.log(
          `⚠️  SUSPICIOUS: Game completed at FLOP round (should be at SHOWDOWN)`
        );
      ...
      if (allInPlayers.length > 0) {
        ...
        allInPlayers.forEach((p) => {
          console.log(
            `   ${p.name} (P${p.position}) - totalBet=$${p.totalBet}`
          );
        });
        ...
        if (gameState.currentRound !== 'showdown') {
          ...
          console.log(
            `   Should have auto-advanced through remaining streets and evaluated at river`
          );
        }
      }
