...
const {
  ...
  distributePots,
  awardPots,
} = require('./backend/lib/pot-manager');
...
function validateChipTotal(label) {
  ...
  if (total !== expected) {
    ...
    console.log(
      `   Player 0: ${state.players[0].chips}, Player 1: ${state.players[1].chips}, Pot: ${state.pot}`
    );
    return false;
  }
  return true;
...
function botDecideAction(playerPosition) {
  ...
  if (actions.canCheck) {
    ...
    actions.canRaise &&
    rand < 0.8 &&
    state.players[playerPosition].chips > actions.callAmount + actions.minRaise
  ) {
...
function playBettingRound() {
  ...
  console.log(
    `   Round: ${state.currentRound}, Current bet: ${state.currentBet}`
  );
...
function playHand() {
  ...
  console.log(
    `Starting chips: P0=${state.players[0].chips}, P1=${state.players[1].chips}, Pot=${state.pot}`
  );
  ...
  for (const street of streets) {
    ...
    console.log(
      `   Community: ${state.communityCards.map((c) => `${c.rank}${c.suit[0]}`).join(' ')}`
    );
  ...
  console.log(
    `   Active players: ${state.players.filter((p) => p.status === PLAYER_STATUS.ACTIVE || p.status === PLAYER_STATUS.ALL_IN).length}`
  );
  ...
  if (state.currentRound === ROUND.RIVER && isBettingRoundComplete(state)) {
    ...
    console.log(`\n📍 SHOWDOWN`);
    console.log(
      `   P0 hole cards: ${state.players[0].holeCards.map((c) => `${c.rank}${c.suit[0]}`).join(' ')}`
    );
    console.log(
      `   P1 hole cards: ${state.players[1].holeCards.map((c) => `${c.rank}${c.suit[0]}`).join(' ')}`
    );
    console.log(
      `   Community: ${state.communityCards.map((c) => `${c.rank}${c.suit[0]}`).join(' ')}`
    );
    ...
    console.log(
      `   Player totalBets: P0=${state.players[0].totalBet}, P1=${state.players[1].totalBet}`
    );

    state = processShowdown(state);

    console.log(
      `   After showdown: P0=${state.players[0].chips}, P1=${state.players[1].chips}, Pot=${state.pot}`
    );
...
try {
  ...
  console.log(`${'='.repeat(60)}`);
  console.log(
    `Final chips: P0=${state.players[0].chips}, P1=${state.players[1].chips}`
  );
  console.log(
    `Total: ${state.players[0].chips + state.players[1].chips} (should be ${STARTING_CHIPS * 2})`
  );
} catch (error) {
  console.log(`\n\n❌ ERROR:`, error.message);
