  ...
  while (true) {
    try {
      ...
      if (game.currentRound && game.handNumber !== hand) {
        ...
        console.log(`\n--- Hand #${hand} ---`);
        console.log(`${bot1.name}: $${player1.chips} | ${bot2.name}: $${player2.chips}`);
        ...
        if (currentGame.currentPlayerPosition === botPlayer.position) {
          ...
          if (validActions && validActions.canAct) {
            ...
            if (bot.strategy === 'aggressive') {
              ...
              } else if (validActions.canRaise && validActions.maxRaise >= 100) {
                action = 'raise';
...
async function main() {
  ...
  try {
    ...
    console.log(`✅ AggressiveBot joined at position ${bot1Login.player?.position}`);
    ...
    console.log(`✅ ConservativeBot joined at position ${bot2Login.player?.position}`);
