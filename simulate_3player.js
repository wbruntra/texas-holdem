...
import {
  ...
  processAction,
  getValidActions,
} from './backend/lib/betting-logic.js';
...
function playHand() {
  ...
  const initialTotal =
    state.players.reduce((s, p) => s + p.chips, 0) + state.pot;
  const chips = state.players.map((p, i) => `P${i}=$${p.chips}`).join(' ');
  console.log(
    `Hand ${handCount}: ${chips} Pot=$${state.pot} (total=$${initialTotal})`
  );
  ...
  if (state.currentRound === ROUND.RIVER && isBettingRoundComplete(state)) {
    const chipsStr = state.players
      .map((p, i) => `P${i}=$${p.chips} (bet=${p.totalBet})`)
      .join(' ');
    ...
    const chipsStr = state.players
      ...
      .join(' ');
    console.log(
      `  Early end: ${chipsStr} Pot=${state.pot} Round=${state.currentRound}`
    );
...
try {
  ...
  console.log(
    `Final: ${state.players.map((p, i) => `P${i}=$${p.chips}`).join(' ')}`
  );
