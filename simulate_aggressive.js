...
function validateChipTotal(label) {
  ...
  if (total !== expected) {
    ...
    console.log(`   P0: ${state.players[0].chips}, P1: ${state.players[1].chips}, Pot: ${state.pot}`);
    return false;
  }
  return true;
}
...
function bot1DecideAction(playerPosition) {
  ...
  } else if (actions.canRaise && state.players[playerPosition].chips >= actions.callAmount + AGGRESSIVE_BET) {
...
function playBettingRound() {
  ...
  while (!isBettingRoundComplete(state) && iterations < maxIterations) {
    ...
    const decision = playerPos === 0 ? bot1DecideAction(playerPos) : bot2DecideAction(playerPos);
    if (!decision) break;
    ...
    if (chipsSpent !== potIncrease) {
      console.log(`   💥 MISMATCH: P${playerPos} ${decision.action} spent $${chipsSpent} but pot only increased $${potIncrease}`);
    }
...
function playHand() {
  ...
  const initialTotal = state.players[0].chips + state.players[1].chips + state.pot;
  console.log(`Hand ${handCount}: P0=$${state.players[0].chips} P1=$${state.players[1].chips} Pot=$${state.pot} (total=$${initialTotal})`);
  console.log(`  (Before blinds: P0 had $${state.players[0].chips + state.players[0].totalBet}, P1 had $${state.players[1].chips + state.players[1].totalBet})`);
  ...
  const afterPreflopActive = state.players.filter(p => 
  ...
  for (const street of streets) {
    ...
    const stillActive = state.players.filter(p => 
      p.status === PLAYER_STATUS.ACTIVE || p.status === PLAYER_STATUS.ALL_IN
    );
  ...
  if (state.currentRound === ROUND.RIVER && isBettingRoundComplete(state)) {
    console.log(`  Showdown: P0=$${state.players[0].chips} (bet=${state.players[0].totalBet}) P1=$${state.players[1].chips} (bet=${state.players[1].totalBet}) Pot=${state.pot}`);
    ...
    // Show winner
    const eval0 = state.players[0].holeCards.length === 2 
      ? evaluateHand(state.players[0].holeCards, state.communityCards) 
      : null;
    const eval1 = state.players[1].holeCards.length === 2 
      ? evaluateHand(state.players[1].holeCards, state.communityCards) 
      : null;
    
    console.log(`  → Winner: P${state.winners[0]} (${eval0 ? eval0.rankName : '?'} vs ${eval1 ? eval1.rankName : '?'})`);
    ...
    console.log(`  Early end: P0=$${state.players[0].chips} (bet=${state.players[0].totalBet}) P1=$${state.players[1].chips} (bet=${state.players[1].totalBet}) Pot=${state.pot} Round=${state.currentRound}`);
...
try {
  while (state.status !== 'completed') {
    ...
    if (handCount % 10 === 0) {
      console.log(`[${handCount} hands] P0=$${state.players[0].chips} P1=$${state.players[1].chips}`);
    }
  ...
  console.log(`\n🎯 GAME OVER!`);
  console.log(`Final state - P0: $${state.players[0].chips}, P1: $${state.players[1].chips}, Pot: $${state.pot}`);
  console.log(`\n✅ SIMULATION COMPLETE`);
  console.log(`Hands played: ${handCount}`);
  console.log(`Final: P0=$${state.players[0].chips} P1=$${state.players[1].chips}`);
  console.log(`Total: $${state.players[0].chips + state.players[1].chips + state.pot} (should be $${STARTING_CHIPS * 2})`);
